...
# 加载tf_infer.py 和tf_train.py中定义的常量和函数。
import tf_infer
import tf_train
...
def evaluate(mnist):
    with tf.Graph().as_default() as g:
        # 定义输入输出的格式。
        x = tf.placeholder(
            tf.float32, [None, tf_infer.INPUT_NODE], name='x-input'
        )
        y_ = tf.placeholder(
            tf.float32, [None, tf_infer.OUTPUT_NODE], name='y-input'
        )
        ...
        # 的函数来获取平均值了。这样就可以完全共用tf_infer.py中定义的
        # 前向传播过程。
        variable_averages = tf.train.ExponentialMovingAverage(
            tf_train.MOVING_AVERAGE_DECAY
        )
